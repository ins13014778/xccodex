...
function getDefaultTeammateModel(leaderModel: string | null): string {
  ...
  if (configured === null) {
    // User picked "Default" in the /config picker â€?follow the leader.
...
async function handleSpawnSplitPane(
  ...
  // Resolve model: 'inherit' â†?leader's model; undefined â†?default Opus
  ...
  // Build the command to spawn xccodex with teammate identity
...
/**
 * Handle spawn operation - creates a new xccodex instance.
