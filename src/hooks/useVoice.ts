...
export function useVoice({
  ...
  function finishRecording(): void {
    ...
    // All ref-backed values are captured here, BEFORE the async boundary �?    // a keypress during the finalize wait can start a new session and reset
    ...
    // bug backend PR #287008 fixes) from "WS handshake never completed" �?    // in the latter case audio is still in audioBuffer, never reached the
      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that xccodex has microphone access.',
            )
          } else {
  ...
  useEffect(() => {
    if (!enabled || !focusMode) {
      // Focus mode was disabled while a focus-driven recording was active �?      // stop the recording so it doesn't linger until the silence timer fires.
