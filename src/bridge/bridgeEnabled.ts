...
// Namespace import breaks the bridgeEnabled â†?auth â†?config â†?bridgeEnabled
// cycle â€?authModule.foo is a live binding, so by the time the helpers below
 ...
 * and Console API logins â€?none of which have the OAuth token CCR needs.
...
export function isBridgeEnabled(): boolean {
  // Positive ternary pattern â€?see docs/feature-gating.md.
 ...
 * config.oauthAccount â€?populated by /api/oauth/profile during login.
...
// program, before enableConfigs() runs. isClaudeAISubscriber() â†?getGlobalConfig()
// throws "Config accessed before allowed" there. Pre-config, no OAuth token can
// exist anyway â€?false is correct. Same swallow getFeatureValue_CACHED_MAY_BE_STALE
 ...
 *
 * This gates which implementation initReplBridge uses â€?NOT whether bridge
...
/**
 * Kill-switch for the `cse_*` â†?`session_*` client-side retag shim.
 *
 ...
 * Defaults to true â€?the shim stays active until explicitly disabled.
 */
 ...
 * in envLessBridgeConfig.ts instead â€?the two implementations have independent
 * version floors.
 *
 * Uses cached (non-blocking) GrowthBook config. If GrowthBook hasn't
 * loaded yet, the default '0.0.0' means the check passes â€?a safe fallback.
 */
export function checkBridgeMinVersion(): string | null {
  // Positive pattern â€?see docs/feature-gating.md.
  // Negative pattern (if (!feature(...)) return) does not eliminate
  // inline string literals from external builds.
  if (feature('BRIDGE_MODE')) {
 ...
 * default â€?the user can still opt out by setting remoteControlAtStartup=false
 * in config (explicit settings always win over this default).
 *
 * Defined here rather than in config.ts to avoid a direct
 * config.ts â†?growthbook.ts import cycle (growthbook.ts â†?user.ts â†?config.ts).
 */
...
/**
 * Opt-in CCR mirror mode â€?every local session spawns an outbound-only
