...
export function autoNameSessionFromPlan(plan: string, setAppState: (updater: (prev: AppState) => AppState) => void, isClearContext: boolean): void {
  ...
  // On clear-context, the current session is about to be abandoned 鈥?its
  ...
  // end with testing steps 鈥?head-slice so Haiku sees the summary.
  [createUserMessage({
    content: plan.slice(0, 1000)
  })], new AbortController().signal).then(async name => {
    // On clear-context acceptance, regenerateSessionId() has run by now 鈥?    // this intentionally names the NEW execution session. Do not "fix" by
...
export function ExitPlanModePermissionRequest({
  ...
  // acceptFeedback when the user approves 鈥?lets users annotate the plan
  ...
  // Hide the Ultraplan button while a session is active or launching 鈥?  // selecting it would dismiss the dialog and reject locally before
  ...
  // only echoes the plan in tool_result when input.plan is set 鈥?otherwise
  ...
  async function handleResponse(value: ResponseValue): Promise<void> {
    ...
    if (value === 'ultraplan') {
      ...
      onDone();
      onReject();
      toolUseConfirm.onReject('Plan being refined via Ultraplan 鈥?please wait for the result.');
    ...
    if (feature('TRANSCRIPT_CLASSIFIER')) {
      ...
      // isAutoModeActive() is the authoritative signal 鈥?prePlanMode/
    ...
    // Handle auto keep-context option 鈥?needs special handling because
  ...
  useLayoutEffect(() => {
    ...
    setStickyFooter(<Box flexDirection="column" borderStyle="round" borderColor="planMode" borderLeft={false} borderRight={false} borderBottom={false} paddingX={1}>
        ...
        {editorName && <Box flexDirection="row" gap={1} marginTop={1}>
            ...
            {isV2 && planFilePath && <Text dimColor> 路 {getDisplayPath(planFilePath)}</Text>}
            {showSaveMessage && <>
          ...
          <Box flexDirection="column" paddingX={1}>
            ...
            {isClassifierPermissionsEnabled() && allowedPrompts && allowedPrompts.length > 0 && <Box flexDirection="column" marginBottom={1}>
                  ...
                  {allowedPrompts.map((p, i) => <Text key={i} dimColor>
                      {'  '}路 {p.tool}({PROMPT_PREFIX} {p.prompt})
                    </Text>)}
                </Box>}
            {!useStickyFooter && <>
                <Text dimColor>
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on xccodex on the web',
      value: 'ultraplan'
    });
  }
  options.push({
