...
function PermissionRulesTab(t0) {
  ...
  if ($[0] !== t0) {
    ...
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
      t8 = {
        allow: "xccodex won't ask before using allowed tools.",
        ask: "xccodex will always ask for confirmation before using these tools.",
        deny: "xccodex will always reject requests to use denied tools."
      };
      $[10] = t8;
    } else {
      t8 = $[10];
...
export function PermissionRuleList(t0) {
  ...
  if ($[89] === Symbol.for("react.memo_cache_sentinel")) {
    t28 = <Text>xccodex can read files in the workspace, and make edits when auto-accept edits is on.</Text>;
    $[89] = t28;
  } else {
    t28 = $[89];
  }
  ...
  if ($[101] !== defaultTab || $[102] !== exitState.keyName || $[103] !== exitState.pending || $[104] !== headerFocused || $[105] !== isSearchMode) {
    t31 = <Box marginTop={1} paddingLeft={1}><Text dimColor={true}>{exitState.pending ? <>Press {exitState.keyName} again to exit</> : headerFocused ? <>鈫?鈫?tab switch 路 鈫?return 路 Esc cancel</> : isSearchMode ? <>Type to filter 路 Enter/鈫?select 路 鈫?tabs 路 Esc clear</> : hasDenials && defaultTab === "recent" ? <>Enter approve 路 r retry 路 鈫戔啌 navigate 路 鈫?鈫?switch 路 Esc cancel</> : <>鈫戔啌 navigate 路 Enter select 路 Type to search 路 鈫?鈫?switch 路 Esc cancel</>}</Text></Box>;
    $[101] = defaultTab;
