...
// feature('KAIROS_CHANNELS'). No feature() guard here 鈥?the whole file
...
export function ChannelsNotice() {
  ...
  if (noAuth) {
    ...
    if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
      t2 = <Text dimColor={true}>Channels require claude.ai authentication 路 run /login, then restart</Text>;
      $[9] = t2;
    } else {
      t2 = $[9];
    }
  ...
  let t2;
  if ($[24] !== flag) {
    t2 = <Text dimColor={true}>Experimental 路 inbound messages will be pushed into this session, this carries prompt injection risks. Restart xccodex without {flag} to disable.</Text>;
    $[24] = flag;
    $[25] = t2;
  } else {
    t2 = $[25];
...
function _temp4(u_0) {
  return <Text key={`${formatEntry(u_0.entry)}:${u_0.why}`} color="warning">{formatEntry(u_0.entry)} 路 {u_0.why}</Text>;
}
function _temp3(u) {
  return <Text key={`${formatEntry(u.entry)}:${u.why}`} color="warning">{formatEntry(u.entry)} 路 {u.why}</Text>;
}
function _temp2(c) {
  return !c.dev;
}
...
function findUnmatched(entries: readonly ChannelEntry[], allowlist: ReturnType<typeof getEffectiveChannelAllowlist>): Unmatched[] {
  ...
  // GrowthBook _CACHED_MAY_BE_STALE 鈥?cold cache yields [] so every plugin
  ...
  // Independent ifs 鈥?a plugin entry that's both uninstalled AND
