...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    ...
    const helpText =
      ...
      '路 Permission denied 鈫?Run: gh auth refresh -h github.com -s repo,workflow\n' +
      '路 Not authorized 鈫?Ensure you have admin access to the repository\n' +
      '路 For manual setup 鈫?Visit: https://github.com/anthropics/claude-code-action'
  ...
  try {
    ...
    if (apiKeyOrOAuthToken) {
      ...
      if (setSecretResult.code !== 0) {
        ...
        const helpText =
          '\n\nNeed help? Common issues:\n' +
          '路 Permission denied 鈫?Run: gh auth refresh -h github.com -s repo\n' +
          '路 Not authorized 鈫?Ensure you have admin access to the repository\n' +
          '路 For manual setup 鈫?Visit: https://github.com/anthropics/claude-code-action'
