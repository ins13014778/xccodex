...
const DEFAULT_PROJECT_RULES_FILE = 'XCCODE.md'
const LEGACY_PROJECT_RULES_FILE = 'CLAUDE.md'

export function getSteps(): Step[] {
  const fs = getFsImplementation()
  const cwd = getCwd()
  const hasProjectRulesFile =
    fs.existsSync(join(cwd, DEFAULT_PROJECT_RULES_FILE)) ||
    fs.existsSync(join(cwd, LEGACY_PROJECT_RULES_FILE))
