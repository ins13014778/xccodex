...
import { getSettingsCandidateFilePathsForSource } from './settings.js'
...
async function getWatchTargets(): Promise<{
  ...
  for (const source of SETTING_SOURCES) {
    ...
    for (const path of getSettingsCandidateFilePathsForSource(source)) {
...
function getSourceForPath(path: string): SettingSource | undefined {
  ...
  return SETTING_SOURCES.find(
    source =>
      getSettingsCandidateFilePathsForSource(source).includes(normalizedPath),
  )
}
