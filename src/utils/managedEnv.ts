...
export function applyProviderProtocolEnvAliases(
  env: NodeJS.ProcessEnv,
): NodeJS.ProcessEnv {
  const protocol = env.XCCODE_PROVIDER_PROTOCOL?.trim().toLowerCase()
  if (!protocol) {
    return env
  }
  ...
  if (protocol === 'anthropic-compatible') {
    delete env.XCCODE_USE_OPENAI_COMPATIBLE
    if (env.XCCODE_BASE_URL) {
      env.ANTHROPIC_BASE_URL = env.XCCODE_BASE_URL
    } else {
      delete env.ANTHROPIC_BASE_URL
    }
    if (env.XCCODE_API_KEY) {
      env.ANTHROPIC_API_KEY = env.XCCODE_API_KEY
    } else {
      delete env.ANTHROPIC_API_KEY
    }
    if (env.XCCODE_MODEL) {
      env.ANTHROPIC_MODEL = env.XCCODE_MODEL
    } else {
      delete env.ANTHROPIC_MODEL
    }
  }
