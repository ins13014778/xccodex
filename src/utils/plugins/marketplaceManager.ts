/**
 * Marketplace manager for xccodex plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // Inner function already logged via logForDebugging. Don't logError here �?    // corrupted user config isn't a xccodex bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `xccodex version. Run 'claude marketplace remove ${name}' and ` +
