 ...
 * ```
 * my-plugin/
 * â”œâ”€â”€ plugin.json          # Optional manifest with metadata
 ...
 * Multiple versions within a single seed â†?ambiguous â†?try next seed.
...
export async function probeSeedCacheAnyVersion(
  ...
  for (const seedDir of getPluginSeedDirs()) {
    // The parent of the version dir â€?computed the same way as
...
export async function copyPluginToVersionedCache(
  ...
  // Seed cache hit â€?return seed path in place (read-only, no copy).
  ...
  if (entry && typeof entry.source === 'string' && marketplaceDir) {
    ...
    try {
      ...
      // Only remap ENOENT from the top-level sourceDir itself â€?nested ENOENTs
...
export async function gitClone(
  ...
  // complete â€?same telemetry-scope discipline as mcpb and marketplace_url.
  logPluginFetch(
    'plugin_clone',
    gitUrl,
    'success',
 ...
 * monorepos this is dramatically cheaper than a full clone â€?the tree objects
 ...
 *
 * The clone is ephemeral â€?it goes into a sibling temp directory and is
...
export async function installFromGitSubdir(
  ...
  // Clone into a sibling temp dir (same filesystem â†?rename works, no EXDEV).
  ...
  try {
    ...
    if (sha) {
      ...
      // lazy-fetched â€?the slow, network-bound step). It doesn't move HEAD;
 ...
 *
 * Introduced to fix a perf regression from the syncâ†’async fs migration: sequential
 * `for { await pathExists }` loops add ~1-5ms of event-loop overhead per iteration.
 * With many plugins Ã— several component types, this compounds to hundreds of ms.
 *
 ...
 * ```
 * plugin-directory/
 * â”œâ”€â”€ plugin.json          # Optional: Plugin manifest
 * â”œâ”€â”€ commands/            # Optional: Custom slash commands
 * â”?  â”œâ”€â”€ build.md         # /build command
 * â”?  â””â”€â”€ test.md          # /test command
 * â”œâ”€â”€ agents/              # Optional: Custom AI agents
 * â”?  â”œâ”€â”€ reviewer.md      # Code review agent
 * â”?  â””â”€â”€ optimizer.md     # Performance optimization agent
 * â””â”€â”€ hooks/               # Optional: Hook configurations
 *     â””â”€â”€ hooks.json       # Hook definitions
 * ```
 *
 * Component detection:
...
export async function createPluginFromPath(
  ...
  if (manifest.commands) {
    // Check if it's an object mapping (record of command name â†?metadata)
 ...
 *
 * Resolves enabledPlugins â†?marketplace entries, runs enterprise policy
 ...
 * and loadAllPluginsCacheOnly is which loader runs â€?discovery and policy
 * are identical.
 */
async function loadPluginsFromMarketplaces({
  ...
  const marketplacePluginEntries = Object.entries(enabledPlugins).filter(
    ([key, value]) => {
      ...
      // Skip built-in plugins â€?handled separately by getBuiltinPlugins()
  ...
  // loading â€?this is a read-only path, so returning {} degrades gracefully.
  ...
  // loads the plugin unchecked â€?a silent fail-open. This guard restores
  // fail-closed: unknown source + active policy â†?block.
  //
  // Allowlist: any value (including []) is active â€?empty allowlist = deny all.
  // Blocklist: empty [] is a semantic no-op â€?only non-empty counts as active.
  ...
  // hot path â€?with N plugins across M marketplaces, the old per-plugin
  ...
  const results = await Promise.allSettled(
    marketplacePluginEntries.map(async ([pluginId, enabledValue]) => {
      ...
      // with NO schema validation â€?if one entry is malformed enough to fail
      // our validation but readable enough for the raw cast, it would load
      // unchecked. Unverifiable source + active policy â†?block, always.
      if (!marketplaceConfig && hasEnterprisePolicy) {
        // We can't know whether the unverifiable source would actually be in
        // the blocklist or not in the allowlist â€?so pick the error variant
...
async function loadPluginFromMarketplaceEntryCacheOnly(
  ...
  if (typeof entry.source === 'string') {
    // Local relative path â€?read from the marketplace source dir directly.
    ...
    // finishLoadingPluginFromPath reads pluginPath â€?its error handling
    // surfaces ENOENT as a load failure, no need to pre-check here.
  } else {
    // External source (npm/github/url/git-subdir) â€?use recorded installPath.
  ...
  // Zip cache extraction â€?must still happen in cacheOnly mode (invariant 4)
  ...
  // Delegate to the shared tail â€?identical to the full loader from here
...
async function loadPluginFromMarketplaceEntry(
  ...
  if (typeof entry.source === 'string') {
    ...
    try {
      ...
      //    3. Marketplace entry version, 4. source.sha (pinned commits â€?the
      //    exact value the post-clone call at cached.gitCommitSha would see),
      //    5. 'unknown' â†?ref-tracked, falls through to clone by design.
      const version = await calculatePluginVersion(
        pluginId,
        entry.source,
        undefined,
      ...
      // Check for cached version â€?ZIP file (zip cache mode) or directory
      const zipPath = getVersionedZipCachePath(pluginId, version)
      if (isPluginZipCacheEnabled() && (await pathExists(zipPath))) {
        ...
        // frozen at image build time â€?no freshness concern, 'whatever's there'
        ...
        // disabled for this session â€?errorsOut.push below surfaces it.
        const seedPath =
        ...
        if (seedPath) {
          ...
          // â€?manifest.version (step 1) outranks gitCommitSha (step 3) â€?          // which would cache at e.g. "2.0.0/" while every warm start
          // probes "{sha12}-{hash}/". Mismatched keys = re-clone forever.
          // Recomputation is only needed when pre-clone was 'unknown'
          // (ref-tracked, no hints) â€?the clone is the ONLY way to learn.
          const actualVersion =
            version !== 'unknown'
              ? version
 ...
 * the rest of the load â€?manifest probe, createPluginFromPath, marketplace
 * entry supplementation â€?is identical. Extracted so the cache-only path
...
async function finishLoadingPluginFromPath(
  ...
  if (!hasManifest) {
    ...
    if (entry.skills) {
      ...
      // (once in a debug log template, once in the if) â€?now called once.
      const checks = await Promise.all(
 ...
 * same name â€?the user explicitly pointed at a directory for this session.
 *
...
export function mergePluginSources(sources: {
  ...
  // OR force-disabled â€?both are admin intent that --plugin-dir must not
  ...
  // plugin where entry.name â‰?manifest.name, this guard will silently miss â€?  // but that's a marketplace misconfiguration that breaks other things too
 ...
 * 1. Session-only plugins (from --plugin-dir CLI flag) â€?override
 ...
 * pointed at a directory for this session â€?that intent beats whatever
...
export const loadAllPlugins = memoize(async (): Promise<PluginLoadResult> => {
  ...
  // getAgentDefinitionsWithOverrides() â€?which now call
  // loadAllPluginsCacheOnly â€?see just-cloned plugins instead of reading
 ...
 *
 * CLAUDE_CODE_SYNC_PLUGIN_INSTALL=1 delegates to the full loader â€?that
 ...
 *
 * Separate memoize cache from loadAllPlugins â€?a cache-only result must
 ...
 *
 * The only difference between the two is which marketplace loader runs â€? * session plugins, builtins, merge, verifyAndDemote, and cachePluginSettings
...
async function assemblePluginLoadResult(
  ...
  // Verify dependencies. Runs AFTER the parallel load â€?deps are presence
...
export function cachePluginSettings(plugins: LoadedPlugin[]): void {
  ...
  // anyway â€?resetting here would waste ~17ms on startup re-reading and
